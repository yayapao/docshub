...
**推荐使用 [expo](https://expo.dev/) 进行开发**，以避免使用原生 API 时的兼容问题。
...
- 支持原生 API，通过 `expo install [package]` 安装原生依赖，比如步数、地理位置、系统通知等
...
#### expo - 解决 nativewind 与 react-native-paper UI 组件样式冲突
...
#### expo - Replace Hermes for the right configuration...
