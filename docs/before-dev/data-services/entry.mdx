...
注意，副本需要与集群对应，即 `replica[index]` 需要与当前集群 shard num 对应。
...
2. 创建分布式表，注意字段要一致，引擎为 `Distributed`，将数据分发到 `errors_analysis_local` 表内
...
```sql
CREATE MATERIALIZED VIEW sentry.mv_errors_analysis_local TO entry.errors_analysis_local
(
    `project_id` UInt64,
    `timestamp` DateTime,
    `event_id` UUID,
