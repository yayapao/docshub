...
如上脚本所示，我们在一行申明 <HighlightWithCode code="#!/usr/bin/env zx" /> 之后，就可以编写相关的执行脚本，通过 <HighlightWithCode code="const res = await $'bash command'" />的形式来执行原生 bash 命令，同时我们会根据用户输入的命令行参数来执行不同的动作。
