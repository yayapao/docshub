...
**一言以蔽之：`npm run` 的本质是执行脚本，而 npm 帮我们处理了寻找可执行脚本的过程。**
...
- 这里的实际指向路径由 `node_modules/umi/package.json` 的 `bin` 属性决定
...
<HighlightWithText>
  Q: 执行脚本时，`usr/bin/env: bad interpreter: No such file or directory`？
</HighlightWithText>
