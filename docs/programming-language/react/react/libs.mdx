...
在 StoryBook 内，将文档分为 stories 和 docs 两大类：

- 前者是描述组件的基本单元，后者可以用于描述组件、或者组件应用文档等
- 写法上存在差别，stories 主要是编写 Meta 和 StoryObj 配置，docs 则更多是开发 React 页面
...
1. 修改打包地址
   ...
   storybook 默认执行 `build` 方法后会将产物输出到 `storybook-static`，如需指定输出地址，可以修改构建脚本 `storybook build -o ./dist`
