...
1. ESM is future. 现在 ESM 就是 ECAMScript 标准模块，而其他模块化解决方案，比如 cjs, UMD, AMD 实际都是个人开发者驱动的，不是标准认可的。

2. 在 Node.js 和 Browser runtime 核心模块统一标准有利于前端生态的发展，因此有很多人去主动推动这个事情；

3. ESM 本身能够带来诸多益处。比如 tree-shaking, top-level await, stric mode, 在 ESM 中，import 是<HighlightWithBadge label="静态引入" />（在加载时被编译，因此需要放在开头）。这种设计让其静态可分析，这为我们在使用/设计/开发分析工具时提供便利，最直观的表现就是有利于 tree-shaking。<HighlightWithBadge label="require" /> 是动态引入（在运行时引入，可以放在任何地方），因此执行 require 时会阻塞当前程序，即 blocking。同时 import 在<HighlightWithBadge label="发起请求" />时也会更加智能。

- import 同样支持 `await import()` 的方式来达到动态引入的目
...
在 joycra-scripts 内，我通过 rollup 来将 .ts 打包成 .mjs，以供脚本进行引用。因为现在属于 Node.js 支持 ESM 的“阵痛期”，开发过程并不是一帆风顺。
