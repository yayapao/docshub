...
当出现无法定位 sourceMap 的问题时，我们首先需要确认用户的匹配路径，即：

1. 将源文件和 sourceMap 文件上传至平台，在文件系统内寻找 sourceMap 文件进行匹配；
2. 开启 `Allow JavaScript Source Fetching` 开关，由 Sentry 去线上寻找 sourceMap 文件进行匹配；
...
通过上面的代码我们可以看到，Sentry 会去请求文件，并将返回结果压缩后写入**缓存**，并立即去请求缓存内容，如果没有请求到缓存文件，会直接抛出 `EventError.TOO_LARGE_FOR_CACHE` 错误。
...
我们发现当请求返回内容的长度大于 `settings.SENTRY_SOURCE_FETCH_TIMEOUT` 时会抛出文件过大异常。我们继续在 `server.py` 内查看默认配置：
...
结合实际情况，我们排除由于 `SENTRY_SOURCE_FETCH_MAX_SIZE` 导致的请求文件失败问题。


#### `setting.SENTRY_CACHE_MAX_VALUE_SIZE` 被重新设置导致缓存受限

<HighlightWithText text="server.py" />{''}
...
这里 `SENTRY_CACHE_MAX_VALUE_SIZE` 被设置为 None 以不限制其他缓存的阈值，即以各个缓存自身的配置为准。
