...
同时，如果遇到 **"Your connection is not private"** 的问题，这是由于本地开发证书不被浏览器信任，可以通过 `brew install mkcert` 和 `yarn mkcert-install` 来安装本地开发信任证书解决。
...
在 Sentry 内，基于 Django 的 [ajango.apps](https://docs.djangoproject.com/zh-hans/4.2/ref/applications/#module-django.apps) 和 [django forms](https://docs.djangoproject.com/zh-hans/4.2/ref/forms/fields/) 来分别**注册应用程序**和**设计表单样式**。
...
完成后，可以启动 Sentry 服务，在 `settings/Integrations` 内可以看到插件已经注册成功。
