...
参考 `relay-server/src/actors/envelopes.rs`，经过 `envelope` 接口上报的数据会根据数据类型写入对应的对列：
- Events and event related items, such as attachments, are always queued together. 事件类型会统一处理，并且返回事件ID
- Sessions and Session batches are always queued separately. 会话相关数据会分批次处理
- Metrics are directly sent to the `EnvelopeProcessor`, bypassing the manager's queue and going straight into metrics aggregation. 指标数据不会进入对列，直接写入指标聚合器

如果对列堆积超过 [Config::event_buffer_size](https://docs.sentry.io/product/relay/options/)，写入对列会失败，这种情况下，relay 会直接返回 400（Too many envelopes (event_buffer_size reached)） 错误，上报数据不会写入对列