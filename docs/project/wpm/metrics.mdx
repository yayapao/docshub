...
- 对于测量规则，我们总结一些关键词帮助记忆：LCP 为起点、5秒安静窗口、没有长任务、不超过2个 GET 请求
...
因为 TBT 衡量 LCP 到 TTI 这段时间内的主线程阻塞时常，比如一个页面，存在3个51ms长任务和存在一个10s的长任务，其 TTI 的值是相同的，但是实际体验却相差甚远，第一种情况的 TBT 为 3ms，第二种情况的 TBT 为 9950ms，此时就能体现出差距
