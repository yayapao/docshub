...
As we all konw, sometime errors can be too much to handle, so i filter the mutiple errors, also support the **filters** for you to define which error can be ignored!

### Getting started

Run `npm install react-error-catcher` or `yarn add react-error-catcher` to install the package.
