...
::: note
记录在开发 up-app 前端时的组件封装、使用方法，便于快速查询，以及项目实施过程的一些注意点
:::
...
3. 在根节点，比如 `baseLayout.tsx` 内引入消息条组件，从而达到提升消息条组件至全局的目的；
...
export default () => {
  ...
  function alertSuccess() {
    const { success, msg } = await getSomething()
    snackOpen({
      content: success ? '订阅成功' : msg,
