...
import { HighlightWithText, HighlightWithCode } from '../../src/components/Highlights'
...
<HighlightWithText>配置无误,但是访问网页时返回 403</HighlightWithText>

403 的情况可能由多种原因导致:

第一，查看 nginx 的启动用户和工作用户是否对应

```bash
# 执行发现工作用户为 root, 而在 nginx.conf 的 user 设置为 www-data
# 将 nginx.conf 的 user 设置为 root 即可
$ ps aux | grep "nginx: worker process" 
```

<HighlightWithText>server_name 配置是正确的，但是访问子域名会被父域名拦截，返回 404</HighlightWithText>

发现是腾讯云 DNS 对子域名的解析配置为“默认”，无法解析，则移交给最上层父域名进行处理，找不到对应资源，返回 404，将 “默认” 修改为 “境内” 解决问题


<HighlightWithText>Incompatible SockJS! Main site uses: "1.4.0", the iframe: "1.3.0".</HighlightWithText>
...
<HighlightWithText>CreateDirectory()...failed (3: The system cannot find the path specified)</HighlightWithText>
