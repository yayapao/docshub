...
- 参考 [腾讯云 nginx 配置 SSL](https://cloud.tencent.com/document/product/400/35244)
- 将服务进行拆分，通过 `` 进行引入
- 支持 SSL 证书和 http 重定向至 https
...
http {
    ...
    server {
        ...
        # 证书文件的名称
        ssl_certificate crt_xxx_com/1_www.xxx.com_bundle.crt;
        # 私钥文件
        ssl_certificate_key crt_xxx_com/2_www.xxx.com.key;
    ...
    # 引入其他服务，其他服务配置文件放在 servers 文件夹下
    include servers/*;
}
```
...
server {
  ...
  # 证书文件的名称
  ssl_certificate crt_xxx_com/1_name_bundle.crt;
  # 私钥文件
  ssl_certificate_key crt_xxx_com/2_name;
  ssl_session_timeout 5m;
  ssl_protocols TLSv1 TLSv1.1 TLSv1.2;
  # 套件加密
  ssl_ciphers ECDHE-RSA-AES128-GCM-SHA256:HIGH:!aNULL:!MD5:!RC4:!DHE;
  ...
  location ^~ /api/xxx/ {
    	# 设置跨域
			if ($request_method = OPTIONS ) {
     				 ...
     				 add_header "Access-Control-Allow-Headers" "Authorization, Origin, X-Requested-With, Content-Type, Accept, x-token, upload-images-type, upload-order-id, User-Agent, Referer, photo-set-id";
     				 return 200;
    			}
	  ...
	  proxy_pass http://127.0.0.1:7727/api/xxx/;
  }

}


# 配置原始访问，如果访问 http，即 80 端口，则重定向 301 (https)
server {
        listen        80;
        server_name api.xxx.com;
        return 301 https://$host$request_uri;
...
**发现 server_name 配置是正确的，但是访问子域名会被父域名拦截，返回 404**
