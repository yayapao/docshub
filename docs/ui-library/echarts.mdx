...
<DocsHeader
  ...
  links={[
    {
      label: 'Documentation - Apache ECharts',
...
ECharts 是一个基于 JavaScript 的图表库，提供直观，生动，可交互，可个性化定制的图表。
...
至此，一切运行完美。**但是当我们在 `ReactEChartsCore` 内使用 `onEvents` 时，会发现只要鼠标滚动，都会触发 chart 的重新渲染，注意：不会重新请求数据，只是重新渲染！**
